...
for i = 1:m
  ...
  J = J + sum(-y_i .* log(h_thetai) - (1 - y_i) .* log(1 - h_thetai))/m;

end

% Regularisation of cost

J = J + lambda * (sum(sumsq(Theta1(:,2:input_layer_size+1))) + sum(sumsq(Theta2(:,2:hidden_layer_size+1))))/(2*m); 
...
for t = 1:m
	...
	delta_3 = a_3t - y_i;
	delta_2 = delta_3 * Theta2 .* sigmoidGradient([1 z_2t]);
	
	delta_accum_1 = delta_accum_1 + delta_2(2:end)' * a_1t;
	delta_accum_2 = delta_accum_2 + delta_3' * a_2t;
